...
function loginByAccessToken(req, res) {
    ...
    request.get(config.uber.profile_url, {
        ...
        if (err) {
            ...
            usersRef.child(data.uuid).once('value', function (snap) {
                if(snap.val()) {
                    console.log('User exists', snap.val());

                    usersRef.child(data.uuid).update({accessToken: access_token, promo_code: data.promo_code}, function(err) {
                        if(!err) {
                    ...
                    usersRef.child(data.uuid).set(data, function(err) {
                        if(!err) {
    ...
    OAuth2.getOAuthAccessToken(query.code, params, function (err, access_token, refresh_token, results) {
        ...
        request.get(config.uber.profile_url, {
            ...
            if (err) {
                ...
                usersRef.child(data.uuid).once('value', function (snap) {
                    if (snap.val()) {
                        console.log('\nUser exists', snap.val());
                        
                        usersRef.child(data.uuid).update({accessToken: access_token, promo_code: data.promo_code, picture: data.picture}, function (err) {
                            if (!err) {
                                return res.send({message: 'User updated', response: data});
                            }
